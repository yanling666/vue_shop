...
export default {
  ...
  data() {
    return {
      ...
      onlyTableData: [],
      //上传图骗得url地址
      uploadUrl: "http://42.192.40.14:8889/api/private/v1/upload",
      //图片上传组件的headers请求头对象
      headObj: {
  ...
  methods: {
    ...
    handleRemove(file) {
      ...
      //3.调用数字的splice方法把图片信息对象从pics数组中移除
      this.addForm.pics.splice(i, 1);
    },
    // 监听图片上传成功的事件
    handleSuccess(responese) {
      //1.拼接得到一个图片信息对象
      const picsInfo = { pics: responese.data.tmp_path };
